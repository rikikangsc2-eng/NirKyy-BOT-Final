...
const callGeminiAPI = async (system, query) => {
...
const command = {
    ...
    async execute({ sock, m, args, text: fullText }) {
        ...
        if (userInput.toLowerCase() === 'oke') {
            ...
            try {
                ...
                const [rawHtml, pageTitle] = await Promise.all([
                    callGeminiAPI(HTML_GENERATOR_SYSTEM_PROMPT, conversationLog),
                    callGeminiAPI(TITLE_GENERATOR_SYSTEM_PROMPT, conversationLog)
                ]);
        ...
        try {
            ...
            const rawResponse = await callGeminiAPI(DISCUSSION_SYSTEM_PROMPT, conversationLog);
