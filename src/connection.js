...
async function connectToWhatsApp() {
    ...
    sock = makeWASocket({
        ...
        getMessage: async (key) => {
            // Adaptasi untuk struktur baru
            const stored = messageStore.get(key.id);
            return stored ? stored.message : undefined;
        }
    });

    // Simpan pesan ke store dan teruskan ke handler
    sock.ev.on('messages.upsert', ({ messages, type }) => {
        ...
        // teruskan ke handler custom
        handleMessageUpsert(sock, { messages, type });
    });

    // Pairing code kalau belum register
